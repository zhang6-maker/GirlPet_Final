...
### 🧠 AI 对话与人格
- 接入本地 [Ollama](https://ollama.com) 大模型，默认 `qwen2:7b`，完全离线可用。
- 内置毒舌傲娇系统提示词，每个回复都带刺儿，但偷偷关心你。
- 支持**用户画像记忆**：记住你的性别、恋爱状态、生日，节日或生日时会主动祝福。
...
- [Python](https://www.python.org) 3.10+ （并安装以下库：`fastapi`, `uvicorn`, `edge-tts`, `vosk`, `numpy`）
- [Ollama](https://ollama.com) 并下载至少一个中文模型（推荐 `qwen2:7b`）
...
- 注意角色有**最小状态持续时间**（默认 10 分钟），防止频繁切换导致闪烁。

---


### 🦊 PPT 功能问题
- **[保存或制作PPT时播放work.gif动画](https://chat.deepseek.com/share/u70fk1tntkjdo20wpb)**  
...
### 🛏️ 宠物睡觉/唤醒 Bug
- **[goToBed 前置检查分析](https://chat.deepseek.com/share/jthqdq4w53464aakq5)**  
...
- **[“睡了醒、醒了睡”循环原因](https://chat.deepseek.com/share/iaskochagjt5bdghbt)**  
  ...
  **根因分析**：  
  ...
  - 手动唤醒时只更新了 UI 状态，但**没有重置 `lastInteractionTime`**，导致下一次定时器检查时立刻判定超时，再次触发睡眠。  
...
### 🧹 `actions.js` 代码质量问题
- **[未定义变量与重复声明分析](https://chat.deepseek.com/share/9gpcnswz7ch4uonxgu)**  
  **涉及文件**：`actions.js`。  
...
### 🔧 代码优化建议
- **[代码优化（方向1）](https://chat.deepseek.com/share/46xobuxiez9w524b9p)**  
  **侧重**：模块化与全局变量治理。  
...
- **[代码优化（方向2）](https://chat.deepseek.com/share/rewihw2a0tm0q0zzsp)**  
  **侧重**：性能与内存管理。  
  **具体问题**：  
...
### ⚠️ 关键崩溃与安全漏洞
- **[Critical 级问题汇总](https://chat.deepseek.com/share/muvgf0mkgnpcduf9np)**  
  **涉及文件**：`main.js`、`package.json`、`config` 相关文件。  
  **具体现象**：  
  1. **启动即闪退**，终端报 `TypeError: Cannot read property 'getPath' of undefined`。  
  2. 潜在 XSS 攻击面：渲染进程可以执行任意系统命令。  
  **根因分析**：  
...
### 📋 项目整体评估（按视角细分）
- **[架构、代码质量、安全性与可维护性](https://chat.deepseek.com/share/k23cg0v5waiso8f425)**  
  盘点全局变量污染、模块循环依赖、缺少分层（渲染/主进程职责混在一起）、日志缺失等问题。

- **[代码质量、架构与安全风险](https://chat.deepseek.com/share/4aje96n0lkxpr7rzxj)**  
  重点指出硬编码路径（Python 脚本位置、模型目录）、同步操作阻塞 UI、异常处理缺失等隐患。

- **[设计缺陷、潜在Bug与安全风险](https://chat.deepseek.com/share/5proobd62y5ides2zn)**  
  暴露状态机设计漏洞（缺少完整状态转换图）、定时器泄漏、Electron 安全配置不当三大类。

- **[长期维护视角的综合评估](https://chat.deepseek.com/share/prsyfgrqwgrlbjvgsj)**  
...
1. **保持模块化**，每个 `.js` 文件职责清晰。
2. **新增功能**尽量独立于已有动作系统，避免过多耦合。
3. 如果想让 AI 更“聪明”，可以微调 `chat.js` 中的系统提示词。
4. 本项目使用 `window` 全局对象进行模块通信，若想改为 ES Module 或其他方式，请统一修改并测试所有引用。
